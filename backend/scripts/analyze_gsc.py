    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            try:
                ...
                print(f"JSON parsing error in GSC API response: {str(e)}. Response preview: {response_text[:200]}...", file=sys.stderr)
                return {}
...
def date_range(days_ago_start, days_ago_end=0):
    ...
    days_ago_end=0 means end date is today (matches GSC's "Last X days" behavior).
    """
...
def pull_summary(token, site, start, end):
    """Overall totals."""
    body = {"startDate": start, "endDate": end, "dimensions": [], "type": "web"}
    print(f"Summary API request body: {json.dumps(body, indent=2)}", file=sys.stderr)
    data = gsc_query(token, site, body)
    print(f"Summary API response: {json.dumps(data, indent=2)}", file=sys.stderr)
    rows = data.get("rows", [{}])
...
def main():
    ...
    _default_out = os.path.join(tempfile.gettempdir(), f"gsc_analysis_{os.getpid()}.json")
