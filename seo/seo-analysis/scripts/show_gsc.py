...
DEFAULT_PATH = os.path.join(tempfile.gettempdir(), f"gsc_analysis_{os.getpid()}.json")
...
def show(path):
    try:
        ...
        print(f"Error reading GSC analysis: {e}", file=sys.stderr)
        return

    # For Claude Desktop: output the JSON directly instead of formatted text
